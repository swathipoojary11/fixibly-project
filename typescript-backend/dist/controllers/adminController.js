...
exports.getAdminUsers = exports.getAdminDashboardAnalytics = exports.getAdminReports = exports.getAdminDashboardStats = exports.getAdminDashboardOverview = exports.getRoleCounts = void 0;
const { supabaseAdmin } = require('../config/supabase');
const normalizeBookingStatus = (status) => {
    const value = `${status || ''}`.trim().toLowerCase();
    if (!value || ['pending', 'new', 'open'].includes(value))
        return 'Pending';
    if (['accepted', 'assigned'].includes(value))
        return 'Assigned';
...
const normalizeRoleName = (role) => {
    ...
    if (!value || ['unknown', 'null'].includes(value))
        return 'Unknown';
    if (['customer', 'customers'].includes(value))
        return 'Customer';
    if (['dispatcher', 'dispatchers'].includes(value))
        return 'Dispatcher';
    if (['technician', 'technicians'].includes(value))
        return 'Technician';
    if (['admin', 'admins'].includes(value))
        return 'Admin';
    return 'Unknown';
};
const normalizeAvailability = (value) => {
    const availability = `${value || ''}`.trim().toLowerCase();
    if (['busy', 'working'].includes(availability))
        return 'Busy';
    if (['offline', 'unavailable'].includes(availability))
        return 'Offline';
    return 'Available';
};
...
exports.getRoleCounts = getRoleCounts;
// 1. Full System Overview
const getAdminDashboardOverview = async (req, res) => {
    try {
        const [{ data: bookings, error: bErr }, { data: technicians, error: tErr }, { data: auditLogs, error: aErr }, { data: notifications, error: nErr }, { data: allUsers, error: uErr }] = await Promise.all([
            supabaseAdmin
                .from('bookings')
...
const getAdminReports = async (req, res) => {
    try {
        ...
        const totalLifetimeRevenue = safePayments.reduce((sum, p) => sum + (Number(p.amount) || 0), 0);
        return res.status(200).json({
            success: true,
            reports: {
...
const getAdminDashboardAnalytics = async (req, res) => {
    try {
        ...
        const lateJobs = safeBookings.filter(b => ['Assigned', 'In Progress', 'On The Way'].includes(normalizeBookingStatus(b.booking_status)) && new Date(b.created_at) < twoHoursAgo).length;
...
module.exports = {
    getAdminDashboardOverview: exports.getAdminDashboardOverview,
    getAdminDashboardStats: exports.getAdminDashboardStats,
    getAdminReports: exports.getAdminReports,
    getAdminDashboardAnalytics: exports.getAdminDashboardAnalytics,
