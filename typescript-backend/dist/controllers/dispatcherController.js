...
exports.acceptEmergencyBroadcast = exports.getActiveBookingsWithLocation = exports.getTechnicianSummaryStats = exports.downgradeEmergency = exports.createManualBooking = exports.triggerEmergencyBroadcast = exports.searchCustomerByPhone = exports.searchCustomers = exports.getDispatcherDashboardStats = exports.updateBookingStatus = exports.getDispatcherProfile = exports.reassignTechnician = exports.assignTechnician = void 0;
const { supabaseAdmin } = require('../config/supabase');
const { createNotification } = require('../services/notificationService');
const { logAuditEvent } = require('../services/auditService');
const getBookingIssue = (booking) => booking?.service_problems?.problem_name ||
...
const assignTechnician = async (req, res) => {
    ...
    try {
        ...
        // H. LOG AUDIT EVENT
        await logAuditEvent(dispatcherUserId, 'DISPATCHER', 'Technician Assignment', `Assigned Tech #${technicianId} (${techDetails.name}) to Booking #${bookingId}`);
...
const reassignTechnician = async (req, res) => {
    ...
    try {
        ...
        await logAuditEvent(dispatcherUserId, 'DISPATCHER', 'Reassignment', `Reassigned Booking #${bookingId} from Tech #${oldTechnicianId} to #${newTechnicianId}`);
...
exports.reassignTechnician = reassignTechnician;
...
exports.getDispatcherProfile = getDispatcherProfile;
...
const updateBookingStatus = async (req, res) => {
    ...
    try {
        ...
        if (booking?.technician_id)
            rolesToNotify.push('TECHNICIAN');
        for (const role of rolesToNotify) {
            let targetUserId = null;
            if (role === 'CUSTOMER')
                targetUserId = booking.customer_id;
            if (role === 'TECHNICIAN')
                targetUserId = techUserId;
            await createNotification({
                recipientRole: role,
                userId: targetUserId,
                bookingId,
...
const getDispatcherDashboardStats = async (req, res) => {
    try {
        ...
        const emergencyJobsList = safeBookings.filter(b => b.emergency_flag === true && b.booking_status !== 'Completed' && b.booking_status !== 'Cancelled');
...
exports.getDispatcherDashboardStats = getDispatcherDashboardStats;
...
const triggerEmergencyBroadcast = async (req, res) => {
    ...
    try {
        ...
        await logAuditEvent(dispatcherUserId, 'DISPATCHER', 'Emergency Downgrade', `Triggered emergency broadcast for Booking #${bookingId}`);
        return res.status(200).json({
            success: true,
...
exports.triggerEmergencyBroadcast = triggerEmergencyBroadcast;
// 9. Manual Booking Creation (Walk-in / Phone customer)
const createManualBooking = async (req, res) => {
    try {
        const { customerId, categoryId, problemId, issueDescription, emergencyFlag, emergencyReason, preferredDate, preferredTime, houseNumber, apartmentName, street, area, city, state, pincode, customerName, customerPhone, dispatcherUserId } = req.body;
        ...
        await logAuditEvent(dispatcherUserId || null, 'DISPATCHER', 'Manual Booking', `Created Booking #${booking.booking_id}`);
...
exports.createManualBooking = createManualBooking;
...
const downgradeEmergency = async (req, res) => {
    ...
    try {
        ...
        if (error)
            throw error;
        await logAuditEvent(dispatcherUserId, 'DISPATCHER', 'Emergency Broadcast', `Downgraded emergency flag for Booking #${bookingId}. Reason: ${reason || 'Not specified'}`);
        return res.status(200).json({
            success: true,
...
exports.downgradeEmergency = downgradeEmergency;
...
exports.getTechnicianSummaryStats = getTechnicianSummaryStats;
...
exports.getActiveBookingsWithLocation = getActiveBookingsWithLocation;
...
const acceptEmergencyBroadcast = async (req, res) => {
    ...
    try {
        ...
        if (bErr || !booking)
            return res.status(404).json({ error: 'Booking not found' });
        if (booking.technician_id)
            return res.status(409).json({ error: 'Already assigned to another technician' });
...
exports.acceptEmergencyBroadcast = acceptEmergencyBroadcast;
module.exports = {
    assignTechnician: exports.assignTechnician,
    reassignTechnician: exports.reassignTechnician,
    downgradeEmergency: exports.downgradeEmergency,
    getTechnicianSummaryStats: exports.getTechnicianSummaryStats,
    updateBookingStatus: exports.updateBookingStatus,
    getDispatcherDashboardStats: exports.getDispatcherDashboardStats,
    triggerEmergencyBroadcast: exports.triggerEmergencyBroadcast,
    searchCustomers: exports.searchCustomers,
    searchCustomerByPhone: exports.searchCustomerByPhone,
    createManualBooking: exports.createManualBooking,
    getDispatcherProfile: exports.getDispatcherProfile,
    getActiveBookingsWithLocation: exports.getActiveBookingsWithLocation,
    acceptEmergencyBroadcast: exports.acceptEmergencyBroadcast
};
//# sourceMappingURL=dispatcherController.js.map