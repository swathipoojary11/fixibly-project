...
const Dashboard = ({ onBack, onNavigate }) => {
  ...
  const kpis = [
    { icon: FiCalendar,      label: "Total Bookings Today",   value: kpi.totalBookingsToday,   color: "orange", nav: "bookings",     trend: "+8.2%" },
    { icon: FiClock,         label: "Pending Bookings",       value: kpi.pendingBookings,      color: "yellow", nav: "bookings",     trend: "-3.1%" },
    { icon: FiTool,          label: "In Progress Jobs",       value: kpi.inProgressJobs,       color: "blue",   nav: "bookings",     trend: "+12.5%" },
    { icon: FiCheckCircle,   label: "Completed Jobs",         value: kpi.completedJobs,        color: "green",  nav: "bookings",     trend: "+15.3%" },
    { icon: FiXCircle,       label: "Cancelled Jobs",         value: kpi.cancelledJobs,        color: "red",    nav: "bookings",     trend: "-2.0%" },
    { icon: FiAlertTriangle, label: "Late Jobs",              value: kpi.lateJobs,             color: "purple", nav: "bookings",     trend: "+5.0%" },
    { icon: FiZap,           label: "Emergency Bookings",     value: kpi.emergencyBookings,    color: "red",    nav: "bookings",     trend: "-1.0%" },
    { icon: FiUsers,         label: "Active Technicians",     value: kpi.activeTechnicians,    color: "cyan",   nav: "technicians",  trend: "" },
    { icon: FiDollarSign,    label: "Expected Revenue Today", value: `₹${kpi.expectedRevenueToday?.toLocaleString()}`, color: "green", nav: "revenue", trend: "+10.2%" },
  ];

  return (
    <div className="space-y-6 animate-fadeIn">
      {/* KPI Grid — each card navigates */}
      <div>
            ...
            return (
              <button key={i} onClick={() => onNavigate(k.nav)}
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  {k.trend && (
                    <span className={`text-xs font-semibold ${k.trend.startsWith("+") ? "text-green-500" : "text-red-500"}`}>{k.trend}</span>
                  )}
                </div>
      ...
      {/* Technician Workload — navigates to technicians page */}
