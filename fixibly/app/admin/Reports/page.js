        ...
        {TABS.map(({ key, label, icon: Icon, color }) => (
          <button key={key} onClick={() => setActive(active === key ? null : key)}
            ...
            <div className={`w-12 h-12 rounded-xl mb-3 flex items-center justify-center ${
              color === "orange" ? "bg-orange-100" : color === "blue" ? "bg-blue-100" : "bg-purple-100"
            }`}>
              <Icon className={`w-6 h-6 ${
                color === "orange" ? "text-primary" : color === "blue" ? "text-blue-600" : "text-purple-600"
              }`} />
            </div>
            <p className="font-bold text-dark-900 mb-1">{label}</p>
            <p className="text-xs text-gray-400">{reports[key].period}</p>
