        ...
        <div className="pb-5 flex-1">
          <div className="ff-card p-4 hover:border-orange-200">
            <div className="flex items-start justify-between gap-2 flex-wrap">
              <div>
                <p className="text-sm font-semibold text-dark-800">{log.action}</p>
                <p className="text-xs text-gray-500 mt-0.5">{log.detail}</p>
              </div>
              <StatusBadge status={log.status} />
            </div>
            <div className="flex items-center gap-4 mt-2 text-xs text-gray-400">
              <span className="flex items-center gap-1"><FiUser className="w-3 h-3" />{log.user}</span>
              <span className="flex items-center gap-1"><FiCalendar className="w-3 h-3" />{new Date(log.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
              {log.bookingId && <span className="font-mono text-primary text-xs">{log.bookingId}</span>}
            </div>
          </div>
        </div>
      </div>
